...
filenames = ['NLO_w0_b6p7.txt', 'NLO_w0_b6p9.txt', 'NLO_w0_b7p2.txt', 'NLO_w0_b7p4.txt', 'NLO_w0_b7p5.txt']
#colors = ['green', 'red', 'blue', 'purple', 'black']
colors = ["b", "g", "orange", "purple", "red"]
labels = ['$\\beta = 6.9$', '$\\beta = 7.05$', '$\\beta = 7.2$', '$\\beta = 7.4$', '$\\beta = 7.5$']
...
for filename, color, label in zip(filenames, colors, labels):
    ...
    plt.errorbar(x_squared, inv_y, yerr=err_inv_y, xerr=err_x_squared, fmt='o', color=color, label=label, capsize=1, markersize=5.7, markerfacecolor='none')
