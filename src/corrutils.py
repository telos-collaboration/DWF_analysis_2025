...
def fold_correlators(correlators):
    ...
    for i in range(1,num_half_slices):
        if correlators[i][1]*correlators[num_time_slices - i][1] < 0:
            folded_correlators[i] = 0.5 * (-correlators[i] + correlators[num_time_slices - i])
        else:
            folded_correlators[i] = 0.5 * (correlators[i] + correlators[num_time_slices - i])
...
def fold_correlators_ZA(correlators):
    ...
    for i in range(0,num_half_slices):
        ...
        folded_correlators[i] = 0.5 * (-correlators[i] + correlators[num_time_slices - 1 - i])
...
def jackknife_effective_mass_block(C1, C2, block_size_fraction=0.2):
    ...
    for i in range(num_blocks):
        ...
        indices_to_keep = np.setdiff1d(np.arange(Nconf), np.arange(start_index, end_index))
...
def jackknife_effective_mass_block4(C1, NT, block_size_fraction=0.1):
    ...
    def find_meff(C_nt, C_nt_plus_1, nt, NT):
        ...
        initial_guess = 0.5
        meff_solution, = fsolve(equation, initial_guess, args=(C_nt, C_nt_plus_1, nt, NT))
...
def jackknife_effective_mass_block_ZA(C1, C2, block_size_fraction=0.1):
    ...
    for i in range(num_blocks):
        ...
        for t in range(0, C1.shape[0] - 1):
            tmp_array[t] = 0.5*( (jackknife_mass_sample1[t+1] + jackknife_mass_sample1[t]) / (2*jackknife_mass_sample2[t+1]) + 2*jackknife_mass_sample1[t+1] / (jackknife_mass_sample2[t+1] + jackknife_mass_sample2[t+2]))
        jackknife_masses[i] = tmp_array
...
def correlated_chi_square(params, measurements, covariance_matrix):
    ...
    for i in range(len(measurements)):
        for j in range(len(measurements)):
            chi_sq += (measurements[i] - avg) * covariance_matrix[i, j] * (measurements[j] - avg)
    return chi_sq


def perform_correlated_fit(effective_mass, covariance_matrix, ti, tf, num_bootstrap=100):
    ...
    for _ in range(num_bootstrap):
        ...
        bootstrap_indices = np.random.choice(len(selected_masses), size=len(selected_masses), replace=True)
        bootstrap_masses = selected_masses[bootstrap_indices]

        # Fit the bootstrap sample
        initial_guess = [np.mean(bootstrap_masses)]
        min_func = lambda params: correlated_chi_square(params, bootstrap_masses, selected_covariance)
