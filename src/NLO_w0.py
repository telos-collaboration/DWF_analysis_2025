...
for N in range(1, 5):
    ...
    err_x = 2 * (m_PS * w0) * np.sqrt((m_PS * w0_err) ** 2 + (w0 * m_PS_err) ** 2)  # Propagate error for x^2
    ...
    plt.errorbar(
        ...
        fmt='o', color=CB_color_cycle[0], label='Data', capsize=1.5, elinewidth=1.5, markersize=6
    )
    ...
    plt.fill_between(x_line, y_fit_lower, y_fit_upper, color=CB_color_cycle[0], alpha=0.3, label='Confidence Band')
    # Labeling the plot
...
for (beta, N), color in zip(beta_N_pairs, colors):
    ...
    plt.errorbar(
        x, y, xerr=err_x, yerr=err_y,
        fmt='o', color=color, label=f'$\\beta = {beta:.1f}$', capsize=2, elinewidth=1.5, markersize=6
    )
