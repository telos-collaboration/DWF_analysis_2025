...
def perform_fit(data):
    ...
    try:
        popt_mM, pcov_mM = curve_fit(
            ...
            a, m2_V, sigma=m2_V_err, absolute_sigma=True, maxfev=5000
        )
...
ensemble_config_sorted = dict(sorted(ensemble_config.items(), key=lambda x: x[1]['beta']))
...
for ens, params in ensemble_config_sorted.items():
    ...
    m_PS_squared_err = 2 * (wf_measurements_data['w_0'] * ensemble_data['m_PS']) * ensemble_data['m_PS_err']
    m_V_squared_err = 2 * (wf_measurements_data['w_0'] * ensemble_data['m_V']) * ensemble_data['m_V_err'] * 7.0

    plt.errorbar(
        m_PS_squared, m_V_squared,
        yerr=m_V_squared_err, xerr=m_PS_squared_err,
        fmt="o", color=params['color'], label=f"$\\beta = {params['beta']}$",
        elinewidth=1.3, markersize=6.1, markeredgecolor="black"
    )

if not np.isnan(popt_mM[0]):
    min_m2_PS = m_PS_squared.min()
    max_m2_PS = m_PS_squared.max()
    m2_PS_range = np.linspace(min_m2_PS - 0.1 * (max_m2_PS - min_m2_PS), max_m2_PS * 2.1, 500)
    m2_V_extrapolated = mM_NLO(popt_mM[0], popt_mM[1], popt_mM[2], popt_mM[3], m2_PS_range, 0)
    m2_V_upper = m2_V_extrapolated + 0.5 * mM_errors[0]
    m2_V_lower = m2_V_extrapolated - 0.5 * mM_errors[0]
    shift = 0.0
    plt.plot(m2_PS_range, m2_V_extrapolated + shift, "black", linestyle="--", linewidth=1.2, label="Extrapolated fit (a = 0)")
    plt.fill_between(m2_PS_range, m2_V_lower  + shift, m2_V_upper  + shift, color='gray', alpha=0.3)
